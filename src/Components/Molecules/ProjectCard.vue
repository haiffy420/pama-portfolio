...
<template>
  <article
    data-aos="fade-up"
    class="rounded-xl mt-10 overflow-hidden shadow-xl shadow-slate-300 dark:shadow-slate-900"
  >
    <slot name="image"></slot>
    <div class="dark:bg-darkCard h-[280px] p-4">
      <h1 class="dark:text-primaryDark truncate font-semibold text-xl pt-1">
        {{ title }}
      </h1>
      <p class="text-content h-[11ch] overflow-hidden pt-4 font-light">
        {{ desc }}
      </p>
      <h3
        class="text-[#666666] dark:text-primaryDark truncate font-medium pt-4"
      >
        Tech Stack : <span class="font-light">{{ stack }}</span>
      </h3>
      <div class="flex justify-around items-center mt-5">
        <div class="flex items-center">
          <svg
            class="stroke-[#666666] dark:stroke-white inline-block min-w-fit"
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
          ...
          <a
            ...
            class="underline pl-2 font-light dark:text-white"
          >
            Live Preview
          </a>
        </div>
